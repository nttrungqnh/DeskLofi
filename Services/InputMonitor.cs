...
public sealed class InputMonitor : IDisposable
{
    ...
    private readonly long _mouseMoveThrottleMs;
    ...
    private const int WhKeyboardLl = 13, WhMouseLl = 14, WmKeyDown = 0x100, WmSysKeyDown = 0x104, WmMouseMove = 0x200, WmLButtonDown = 0x201, WmRButtonDown = 0x204, WmMButtonDown = 0x207, WmMouseWheel = 0x20A, WmXButtonDown = 0x20B, WmMouseHWheel = 0x20E;
    ...
    public InputMonitor(int mouseMoveThrottleMs = 100)
    {
        _mouseMoveThrottleMs = Math.Max(0, mouseMoveThrottleMs);
        _hasLastMousePosition = GetCursorPos(out _lastMousePosition);
    ...
    private IntPtr MouseCallback(int code, IntPtr w, IntPtr l)
    {
        int m = w.ToInt32();
        if (code >= 0 && (m == WmMouseMove || m == WmLButtonDown || m == WmRButtonDown || m == WmMButtonDown || m == WmXButtonDown || m == WmMouseWheel || m == WmMouseHWheel))
        {
            var position = Marshal.PtrToStructure<MouseData>(l).Point;
            var moved = !_hasLastMousePosition || position.X != _lastMousePosition.X || position.Y != _lastMousePosition.Y;
            _lastMousePosition = position;
            _hasLastMousePosition = true;
            if (m != WmMouseMove || moved)
            {
                long now = Environment.TickCount64, previous = Interlocked.Read(ref _lastMouseActivityTicks);
                if (m != WmMouseMove || now - previous >= _mouseMoveThrottleMs)
                {
                    Interlocked.Exchange(ref _lastMouseActivityTicks, now);
                    MouseActivity?.Invoke(DateTime.UtcNow);
                }
            }
        }
