...
public partial class MusicLibraryWindow : Window
{
    ...
    public MusicLibraryWindow(MusicService music){InitializeComponent();_music=music;_music.LibraryChanged+=Refresh;Refresh();}
    private void Refresh(){if(!Dispatcher.CheckAccess()){Dispatcher.BeginInvoke(Refresh);return;}TrackList.ItemsSource=null;TrackList.ItemsSource=_music.Tracks;CountLabel.Text=$"{_music.Tracks.Count} tracks · files remain in their original locations";}
